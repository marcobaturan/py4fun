...
# from: https://www.openbookproject.net/py4fun/logic2/logic2.html
...
class LC:
    # Logic Circuits names and an eval function defined in child classes
...
def test4Bit(a, b):  # a, b four char strings like '0110'
    ...
    F0.Cin.set(0)
    F0.A.set(bit(a, 3));
    F0.B.set(bit(b, 3))  # bits in lists reversed
    F1.A.set(bit(a, 2));
    F1.B.set(bit(b, 2))
    F2.A.set(bit(a, 1));
    F2.B.set(bit(b, 1))
    ...
    print("carry %d" % F3.Cout.value)
    print("sum %d%d%d%d" % (F3.S.value, F2.S.value, F1.S.value, F0.S.value))
